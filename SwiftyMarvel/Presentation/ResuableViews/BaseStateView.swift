...
where VM: ViewModel, SuccessView: View, NoItemsView: View, ErrorView: View,
    ...
    /// Initialize the view with given [ViewModel] and views to display in each state.
    ///
    /// - Parameters:
    /// - viewModel: The [ViewModel] to observe it state.
    ...
    init(viewModel: VM,
         ...
         @ViewBuilder emptyView: @escaping () -> NoItemsView = { MessageView(message: "noDataFound".localized()) },
