...
/// Resolver is singleton class that is responsible for injecting all dependencies in the app
class Resolver {
    ...
    ///
    ///  - Parameter type: The type of dependency to be resolved.
...
extension Resolver {
    private func injectUtils() {
        ...
        container.register(RequestManager.self) { resolver in
            DefaultRequestManager(networkManger: resolver.resolve(NetworkManager.self)!)
        }.inObjectScope(.container)
...
extension Resolver {
    private func injectDataSources() {
        container.register(CharactersDataSource.self) { resolver in
            DefaultCharactersDataSource(requestManger: resolver.resolve(RequestManager.self)!)
        }.inObjectScope(.container)

        container.register(ComicsDataSource.self) { resolver in
            DefaultComicsDataSource(requestManger: resolver.resolve(RequestManager.self)!)
        }.inObjectScope(.container)
